# -*- coding: utf-8 -*-
"""Robo definitivo da Jala(grupo 1)2.0(Quase 100%)
...
class Robot:
    ...
    def ataque(self, robo_inimigo, parte_atacante, parte_atacada):
        ...
        # Reduz a defesa da parte atacada em nívelAtaque
        robo_inimigo.parts[parte_atacada].reduzir_defesa(nivelAtaque)

        # Verifica se a defesa da parte atacada chegou a zero
        if robo_inimigo.parts[parte_atacada].nivelDefesa == 0:
            # Se a defesa chegou a zero, aplica o dano diretamente na energia
            robo_inimigo.energia -= self.parts[parte_atacada].energiaConsumida
        else:
            # Se ainda há defesa, calcula o dano com base na diferença entre ataque e defesa
        ...
        # Verifica se a energia do robô inimigo chegou a zero
        if robo_inimigo.energia < 0:
            robo_inimigo.energia = 0

        # Reduz a energia consumida da parte atacante
        self.energia -= self.parts[parte_atacada].energiaConsumida

        # Verifica se a energia da parte atacante chegou a zero
        if self.energia < 0:
            self.energia = 0
